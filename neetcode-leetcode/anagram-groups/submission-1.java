...
class Solution {
  ...
  /*
   * Solution:
   * Iterate through the array, sort each word into alphabetical order, then use
   * that as a key in a hashmap to
   * store an array list that will track the original
   *
   * Time complexity:
   * O(m * n * log(n)) where m is # of strings and n is length of the longest
   * string
   * Because: for loop of each string means minimum time complexity M, plus in
   * each loop sort is called on each string which is n log n,
   * so m loops times n log n for the operations of each loop means O(m * n *
   * log(n))
   */
  ...
  public List<List<String>> groupAnagrams(String[] strs) {
    ...
    for (String s : strs) {
      // turn each string in the list into a char array to sort then back into a
      // string as a key
