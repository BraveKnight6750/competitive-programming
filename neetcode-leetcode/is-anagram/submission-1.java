...
class Solution {
  ...
  /*
   * Solution:
   * Turn the strings into arrays, sort, and check if they are equal. Works
   * because anagrams when aranged in alphabetical order are always equal
   *
   * Time Complexity:
   * O(n * log(n) + m * log(m)) where m is the length of s and n is the length of
   * t,
